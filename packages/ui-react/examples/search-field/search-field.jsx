...
import { Icon, Input, ButtonIcon, Dropdown, ListGroup } from '../../src';
...
const searchItems = (items, query) => {
  ...
  return items.filter(
    (item) => pattern.test(item.name) || pattern.test(item.company) || pattern.test(item.email)
  );
};
...
export const SearchField = ({ items, ...restProps }) => (
  <Downshift {...restProps} itemToString={itemToString}>
    {({
      getInputProps,
      isOpen,
      reset,
